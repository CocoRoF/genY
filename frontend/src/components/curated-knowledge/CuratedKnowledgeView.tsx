...
// ─── Main View ────────────────────────────────────────────────
export default function CuratedKnowledgeView() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [indexRes, graphRes] = await Promise.all([
        curatedKnowledgeApi.getIndex(),
        curatedKnowledgeApi.getGraph(),
      ]);
      ...
      console.error('Failed to load curated knowledge data:', err);
    } finally {
      setLoading(false);
    }
  ...
  const handleSelectFile = useCallback(
    async (filename: string) => {
      openFile(filename);
      try {
        const detail = await curatedKnowledgeApi.readFile(filename);
        setFileDetail(detail);
      } catch (e) {
        console.error('Failed to read curated file:', e);
      }
    },
  ...
  return (
    <div className="obsidian-root">
      ...
      {/* Create note FAB */}
      <CuratedCreateNoteModal onCreated={loadData} />
    </div>
  );
}
...
function CuratedNoteEditor({
  ...
  const handleDelete = async () => {
    ...
    try {
      await curatedKnowledgeApi.deleteFile(selectedFile);
      onRefresh();
    } catch (e) {
    ...
    <div style={{ padding: '20px 32px', maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        <h1 style={{ flex: 1, fontSize: 22, fontWeight: 700, margin: 0, color: 'var(--obs-text)' }}>
          <Sparkles size={16} style={{ color: 'var(--obs-purple-bright)', marginRight: 8, verticalAlign: 'middle' }} />
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 20 }}>
        ...
        <span style={{
          ...
          background: 'rgba(139,92,246,0.15)', color: 'var(--obs-purple-bright)',
          fontWeight: 600,
        }}>
...
function CuratedSearchView({
  ...
  const handleSearch = async () => {
    ...
    onSetSearching(true);
    try {
      const res = await curatedKnowledgeApi.search(query.trim());
        ...
        {results.map((r, i) => (
          ...
          >
            <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 4 }}>
              <Sparkles size={11} style={{ opacity: 0.5, marginRight: 4 }} />
...
// ─── Create Note Modal ────────────────────────────────────────
function CuratedCreateNoteModal({ onCreated }: { onCreated: () => void }) {
  const { t } = useI18n();
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await curatedKnowledgeApi.createFile({
        title: title.trim(),
