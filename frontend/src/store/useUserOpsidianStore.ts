...
export interface UserOpsidianState {
  ...
  setStats: (s: UserOpsidianState['stats']) => void;
...
const initialState = {
  ...
  stats: null as UserOpsidianState['stats'],
  loading: false,
...
export const useUserOpsidianStore = create<UserOpsidianState>((set) => ({
  ...initialState,
  ...
  closeFile: (fn) =>
    set((state) => {
      const next = state.openFiles.filter((f) => f !== fn);
      return {
        openFiles: next,
        selectedFile: state.selectedFile === fn ? (next[next.length - 1] ?? null) : state.selectedFile,
