...
LLM_CALL_I18N = {
    ...
    "ko": NodeI18n(
        label="LLM 호출",
        description="범용 LLM 호출 노드. 구성 가능한 프롬프트 템플릿을 {field} 상태 변수 치환과 함께 모델에 전송합니다. 조건부 프롬프트 전환, 다중 출력 필드 매핑, 선택적 완료 플래그를 지원합니다.",
        parameters={
            "prompt_template": {
                "label": "프롬프트 템플릿",
                "description": (
                    "모델에 전송할 프롬프트. {field_name} 형식으로 상태 변수를 치환합니다. "
                    "사용 가능 필드: input, answer, review_feedback, last_output 등."
                ),
            },
            "conditional_field": {
                "label": "조건부 프롬프트 필드",
                "description": "프롬프트 전환을 위해 확인할 상태 필드. 설정 후 조건 충족 시 대체 프롬프트가 사용됩니다.",
            },
            "conditional_check": {
                "label": "조건 검사",
                "description": "조건부 필드를 평가하는 방법.",
            },
            "alternative_prompt": {
                "label": "대체 프롬프트",
                "description": "조건부 필드 검사를 통과했을 때 사용되는 프롬프트.",
            },
            "output_field": {
                "label": "출력 상태 필드",
                "description": "모델 응답을 저장할 상태 필드.",
            },
            "output_mappings": {
                "label": "추가 출력 매핑 (JSON)",
                "description": "응답에서 설정할 추가 상태 필드. 키는 필드 이름, 값은 true로 복사.",
            },
            "set_complete": {
                "label": "완료 표시",
                "description": "실행 후 is_complete=True 설정.",
            },
        },
        ...
        help=_help(
            ...
            [
                ("개요", (
                    ...
                    "프롬프트를 Claude에 전송하고 응답을 구성 가능한 상태 필드에 저장합니다.\n\n"
                    "분류나 리뷰 같은 전문 카테고리에 맞지 않는 유연한 범용 모델 호출이 "
                    "필요할 때 이 노드를 사용하세요."
                )),
                ("프롬프트 템플릿", (
                    "프롬프트 템플릿은 `{field_name}` 구문을 사용한 **상태 변수 치환**을 지원합니다.\n\n"
                    "**사용 가능 변수:**\n"
                    "- `{input}` — 원본 사용자 요청\n"
                    "- `{answer}` — 최신 생성된 답변\n"
                ...
                ("출력 설정", (
                    ...
                    "기본값은 `last_output`이며 `answer`, `final_answer` 또는 커스텀 필드로 설정 가능합니다.\n"
                    "- **완료 표시**: 활성화하면 이 노드 실행 후 `is_complete=True`가 설정되어 "
                    "다운스트림 게이트와 실행기에 완료를 알립니다."
                )),
                ("사용 팁", (
                    "1. 다단계 추론을 위해 여러 LLM 호출 노드를 체이닝하세요.\n"
                    "2. 중간 결과를 분리하려면 각기 다른 `output_field` 값을 사용하세요.\n"
                    "3. 컨텍스트 오버플로를 방지하려면 이 노드 앞에 Context Guard를 배치하세요.\n"
                    "4. 완료 신호 감지와 트랜스크립트 기록을 위해 뒤에 Post Model 노드를 배치하세요."
                )),
            ],
        ),
    ),
...
CLASSIFY_I18N = {
    ...
    "ko": NodeI18n(
        label="분류",
        description="범용 LLM 분류 노드. 모델에 프롬프트를 전송하고 응답을 구성된 카테고리로 파싱한 후 일치하는 출력 포트로 실행을 라우팅합니다. 기본 카테고리는 easy/medium/hard이지만 어떤 레이블 세트로도 완전히 커스터마이즈 가능합니다.",
        parameters={
            "prompt_template": {
                "label": "분류 프롬프트",
                "description": "분류를 위해 모델에 전송할 프롬프트. {input}으로 사용자 요청을 치환합니다.",
            },
            "categories": {
                "label": "카테고리",
                "description": "분류 카테고리 이름 (쉼표 구분). 각 카테고리가 라우팅용 출력 포트가 됩니다.",
            },
            "default_category": {
                "label": "기본 카테고리",
                "description": "모델 응답이 구성된 카테고리와 일치하지 않을 때의 대체 카테고리.",
            },
            "output_field": {
                "label": "출력 상태 필드",
        ...
        help=_help(
            "분류 노드",
            "LLM 분석을 통해 입력을 구성 가능한 카테고리로 분류하고 일치하는 출력 포트로 실행을 라우팅합니다.",
            [
                ("개요", (
                    "이것은 의사결정 허브 역할을 하는 **조건부 모델 노드**입니다. "
                    "사용자의 입력을 분류 프롬프트와 함께 모델에 전송하고, "
                    "카테고리 키워드를 파싱한 후 일치하는 출력 포트로 "
                    "실행을 라우팅합니다.\n\n"
                    "기본 카테고리(easy/medium/hard)는 난이도 기반 라우팅에 매핑되지만, "
                    "**어떤 분류 작업이든** 커스텀 카테고리 세트를 구성할 수 있습니다:\n"
                    "- 감정: positive / negative / neutral\n"
                    "- 우선순위: low / medium / high / critical\n"
                    "- 유형: question / request / complaint / feedback"
                )),
                ("분류 작동 방식", (
                    "1. 구성된 프롬프트가 모델에 전송됩니다 (`{input}` 치환).\n"
                    "2. 모델이 구조화된 ``ClassifyOutput`` 객체로 응답하도록 **JSON 스키마 제약**이 자동 주입됩니다.\n"
                    "3. 응답이 JSON으로 파싱되고 Pydantic 스키마로 검증됩니다.\n"
                    "4. ``classification`` 필드가 구성된 카테고리 중 하나에 일치하도록 강제됩니다 (대소문자 무시).\n"
                    "5. JSON 파싱 실패 시 자동으로 **수정 프롬프트**가 전송됩니다.\n"
                    "6. 오류 발생 시 **end** 포트로 라우팅됩니다.\n\n"
                    "분류 결과는 구성된 상태 필드에 저장됩니다 (기본값: `difficulty`)."
                )),
                ("커스터마이즈", (
                    "- **카테고리**: 쉼표로 구분된 카테고리 이름 입력 (예: `low, medium, high, critical`). "
                    "각 카테고리가 자동으로 출력 포트가 됩니다.\n"
                    "- **프롬프트**: 모델에게 카테고리 이름 중 하나로 정확히 응답하도록 "
                    "도메인 특화 프롬프트를 작성하세요.\n"
                    "- **출력 필드**: `difficulty` 외에 다른 상태 필드에도 결과를 저장할 수 있습니다.\n"
                    "- **기본값**: 모호한 응답에 대한 대체 카테고리를 설정하세요."
                )),
                ("사용 팁", (
                    "1. 'end' 포함 **모든 출력 포트**를 유효한 다운스트림 노드에 연결하세요.\n"
                    "2. 이 노드가 직접 라우팅합니다 — 별도의 ConditionalRouter가 필요 없습니다.\n"
                    "3. 토큰 예산 확인을 위해 이 노드 앞에 Context Guard를 배치하세요.\n"
                    "4. 분류기에 컨텍스트를 제공하려면 앞에 Memory Inject를 배치하세요."
                )),
            ],
        ),
    ),
...
DIRECT_ANSWER_I18N = {
    ...
    "ko": NodeI18n(
        label="직접 답변",
        description="리뷰 없이 단발성 직접 답변을 생성합니다. 품질 검사가 필요 없는 쉬운 작업에 적합합니다. 구성 가능한 출력 필드에 응답을 기록하고 워크플로우를 완료로 표시할 수 있습니다.",
        parameters={
            "prompt_template": {
                "label": "프롬프트 템플릿",
                "description": "프롬프트 템플릿. {input}은 사용자 요청입니다.",
            },
            "output_fields": {
                "label": "출력 필드 (JSON)",
                "description": "응답을 저장할 상태 필드. 예: [\"answer\", \"final_answer\"]",
            },
            "mark_complete": {
                "label": "완료 표시",
                "description": "실행 후 is_complete=True 설정.",
        ...
        help=_help(
            "직접 답변 노드",
            "리뷰나 반복 없이 단순 작업에 대한 단발성 답변을 생성합니다.",
            [
                ("개요", (
                    "직접 답변 노드는 자율 워크플로우의 **쉬운 경로**를 처리합니다. "
                    "단일 응답을 생성하고 즉시 작업을 완료로 표시합니다.\n\n"
                    "이것은 가장 빠른 실행 경로입니다 — 리뷰 루프도, TODO 분해도 없습니다. "
                    "간단한 질문, 조회, 단순 요청에 적합합니다."
                )),
                ("프롬프트 설정", (
                    "프롬프트 템플릿은 `{input}` — 사용자의 원본 요청을 받습니다.\n\n"
                    "리뷰 단계가 없으므로 프롬프트를 최대한 명확하고 완전하게 작성하세요. "
                    "필요하다면 출력 형식 지침을 포함시키세요.\n\n"
                    "**예시:**\n"
                    "```\n명확하고 간결한 답변을 제공하세요:\n{input}\n```"
                )),
                ("상태 업데이트", (
                    ...
                    "- `last_output` — 원본 모델 응답"
                )),
                ("사용 팁", (
                    "1. Classify Difficulty의 'Easy' 포트에서 연결하세요.\n"
                    "2. 트랜스크립트 기록을 위해 Post Model 노드를 뒤에 배치하세요.\n"
                    "3. 리뷰 루프는 필요 없습니다 — 이 경로는 속도를 위해 설계되었습니다.\n"
                    "4. 품질이 중요하다면 Medium 경로로 라우팅하세요."
                )),
            ],
        ),
    ),
...
ANSWER_I18N = {
    ...
    "ko": NodeI18n(
        label="답변",
        description="선택적 리뷰 피드백 통합으로 반복 개선을 위한 답변을 생성합니다. 재시도 시 피드백 컨텍스트가 포함된 재시도 템플릿으로 자동 전환됩니다. 예산 인식 프롬프트 압축을 지원합니다.",
        parameters={
            "prompt_template": {
                "label": "프롬프트 템플릿",
                "description": "초기 답변을 위한 프롬프트.",
            },
            "retry_template": {
                "label": "재시도 프롬프트 템플릿",
                "description": "리뷰 거부 후 재시도 시 사용되는 프롬프트 템플릿.",
            },
            "feedback_field": {
                "label": "피드백 상태 필드",
                "description": "리뷰 피드백이 포함된 상태 필드.",
            },
            "count_field": {
                "label": "리뷰 횟수 상태 필드",
                "description": "리뷰 사이클 횟수를 추적하는 상태 필드.",
            },
            "output_fields": {
                "label": "출력 필드 (JSON)",
                "description": "응답을 저장할 상태 필드.",
            },
        },
        output_ports={"default": {"label": "다음"}},
        ...
        help=_help(
            "답변 노드",
            "중간 복잡도 작업에 대한 답변을 생성하며, 재시도 시 리뷰 피드백을 통합합니다.",
            [
                ("개요", (
                    "답변 노드는 자율 워크플로우의 **중간 경로**를 처리합니다. "
                    "피드백 루프를 통해 리뷰되고 개선될 수 있는 답변을 생성합니다.\n\n"
                    "첫 실행에서는 메인 프롬프트 템플릿을 사용합니다. "
                    "후속 재시도(리뷰 거부 후)에서는 리뷰 피드백이 포함된 "
                    "재시도 템플릿으로 전환됩니다."
                )),
                ("프롬프트 템플릿", (
                    "**초기 프롬프트**: 첫 답변 시도에 사용됩니다. `{input}`을 받습니다.\n\n"
                    "**재시도 프롬프트**: 리뷰 거부 시 사용됩니다. 다음을 받습니다:\n"
                    "- `{input_text}` — 원본 요청\n"
                    "- `{previous_feedback}` — 리뷰 피드백\n\n"
                    "`review_count > 0`일 때 재시도 템플릿이 자동 활성화됩니다."
                )),
                ("리뷰 통합", (
                    "이 노드는 **Review** 노드와 함께 작동하도록 설계되었습니다:\n\n"
                    "1. Answer가 응답을 생성합니다\n"
                    "2. Review가 응답을 평가합니다\n"
                    "3. 거부되면 피드백과 함께 Answer가 다시 호출됩니다\n"
                    "4. 승인되거나 최대 재시도 횟수에 도달할 때까지 반복됩니다"
                )),
                ("사용 팁", (
                    "1. Classify Difficulty의 'Medium' 포트에서 연결하세요.\n"
                    "2. 품질 보증을 위해 Review 노드를 뒤에 배치하세요.\n"
                    "3. Review의 'Retry' 포트를 이 노드로 다시 연결하세요.\n"
...
REVIEW_I18N = {
    ...
    "ko": NodeI18n(
        label="리뷰",
        description=(
            "LLM을 통해 생성된 답변을 검토하고 구성 가능한 판정 포트 세트로 라우팅하는 자체 라우팅 품질 게이트. "
            "Pydantic 검증된 구조화 JSON 출력으로 신뢰성 있는 판정 및 피드백 파싱을 수행합니다. "
            "구성된 각 판정이 출력 포트가 됩니다. "
            "무한 루프 방지를 위해 최대 재시도 후 첫 번째 판정을 강제합니다."
        ),
        parameters={
            "prompt_template": {
                "label": "리뷰 프롬프트",
                "description": "품질 리뷰를 위한 프롬프트 템플릿.",
            },
            "verdicts": {
                "label": "판정 (JSON)",
                "description": (
                    "LLM이 내릴 수 있는 판정 이름 목록. 각각이 출력 포트가 됩니다. "
                    '예: ["approved", "retry"] 또는 ["pass", "minor_fix", "major_rewrite"]'
                ),
            },
            "default_verdict": {
                "label": "기본 판정",
                "description": "모델 응답이 구성된 판정과 일치하지 않을 때 사용할 판정.",
            },
            "output_field": {
                "label": "출력 상태 필드",
                "description": "일치된 판정 문자열을 저장할 상태 필드.",
            },
            "max_retries": {
                "label": "최대 리뷰 재시도",
        ...
        help=_help(
            "리뷰 노드",
            "답변을 검토하고 판정별로 라우팅하는 자체 라우팅 품질 게이트 — Classify처럼 각 판정이 포트가 됩니다.",
            [
                ("개요", (
                    "리뷰 노드는 생성된 답변의 품질을 평가하는 **자체 라우팅 조건부 노드**입니다. "
                    "질문과 답변을 모델에 보내 평가하고, "
                    "구조화된 JSON 판정/피드백 응답을 파싱합니다.\n\n"
                    "Classify 노드처럼 **Verdicts** 매개변수가 ``generates_ports``를 사용합니다: "
                    "구성된 각 판정이 명명된 출력 포트가 됩니다. "
                    "오류를 위한 **End** 포트가 항상 포함됩니다.\n\n"
                    "**기본 포트 (기본 판정 사용 시):**\n"
                    "- **승인** — 답변이 품질 기준 충족\n"
                    "- **재시도** — 답변 개선 필요 (피드백 제공)\n"
                    "- **종료** — 오류 또는 조기 종료"
                ...
                ("구성 가능한 판정", (
                    "**Verdicts** 매개변수는 판정 이름의 JSON 목록을 받습니다:\n\n"
                    '- 기본값: ``["approved", "retry"]``\n'
                    '- 커스텀: ``["pass", "minor_fix", "major_rewrite"]``\n\n'
                    "각 판정이 출력 포트가 됩니다. 구조화 출력이 LLM의 "
                    "``verdict`` 필드가 구성된 값 중 하나와 정확히 일치하도록 보장합니다. "
                    "강제 변환 후에도 일치하지 않으면 **기본 판정**이 사용됩니다."
                )),
                ("리뷰 형식", (
                    "모델은 ``verdict``와 ``feedback`` 필드를 포함한 **구조화 JSON 출력**으로 "
                    "응답해야 합니다:\n\n"
                    '```json\n{"verdict": "approved", "feedback": "좋은 답변입니다."}\n```\n\n'
                    "노드는 출력 형식 강제를 위해 Pydantic 스키마 검증을 사용합니다. "
                    "초기 응답이 검증에 실패하면 자동 수정 재시도가 시도됩니다."
                )),
                ("최대 재시도", (
                    "**최대 리뷰 재시도** 매개변수는 무한 리뷰 루프를 방지합니다. "
                    "이 횟수만큼 사이클 후 모델 출력과 관계없이 "
                    "목록의 **첫 번째 판정** (보통 'approved')이 강제됩니다.\n\n"
                    "기본값: 3 사이클. 범위: 1–10."
                )),
                ("사용 팁", (
                    "1. Answer 노드 뒤에 연결하세요 (선택적으로 Guard를 거쳐).\n"
                    "2. 판정 포트를 직접 연결하세요 — 별도의 라우터 노드 불필요.\n"
                    "3. 'Retry'를 Guard 노드로 다시 연결하여 답변 루프를 재진입하세요.\n"
                    "4. 리뷰 피드백은 ``review_feedback`` 상태 필드에 저장됩니다.\n"
                    "5. 리뷰 사이클 횟수는 구성된 Count Field에서 추적됩니다."
                )),
            ],
        ),
    ),
...
CONDITIONAL_ROUTER_I18N = {
    ...
    "ko": NodeI18n(
        label="조건부 라우터",
        description="순수 상태 기반 라우팅 노드. 지정된 상태 필드를 읽고 JSON 라우트 맵을 통해 값을 출력 포트에 매핑합니다. 열거형, 문자열 등을 자동 정규화합니다.",
        parameters={
            "routing_field": {
                "label": "라우팅 상태 필드",
                "description": "라우팅 결정을 위해 읽을 상태 필드 이름.",
            },
            "route_map": {
                "label": "라우트 매핑 (JSON)",
                "description": (
                    "필드 값을 출력 포트 ID에 매핑하는 JSON 객체. "
                    '예: {"value1": "port_a", "value2": "port_b"}'
                ),
            },
            "default_port": {
                "label": "기본 포트",
                "description": "필드 값이 어떤 라우트에도 일치하지 않을 때 사용할 포트.",
            },
        },
        output_ports={"default": {"label": "기본", "description": "대체 라우트"}},
        groups={"routing": "라우팅"},
        help=_help(
            "조건부 라우터 노드",
            ...
            [
                ("개요", (
                    ...
                    "전문 조건부 노드(Classify Difficulty, Review)와 달리, "
                    "이 라우터는 모든 상태 필드에 대해 자체 라우팅 로직을 정의할 수 있습니다."
                )),
                ("라우트 매핑", (
                    "라우팅 규칙을 JSON 객체로 정의합니다:\n\n"
                    "```json\n{\n  \"easy\": \"simple_path\",\n  \"medium\": \"standard_path\",\n  \"hard\": \"complex_path\"\n}\n```\n\n"
                    "**키**는 라우팅 필드의 가능한 값입니다.\n"
                    "**값**은 라우팅할 출력 포트 ID입니다.\n\n"
                    "출력 포트는 라우트 맵 값에서 자동 생성됩니다."
                )),
                ("기본 포트", (
                    "상태 필드 값이 맵의 어떤 라우트에도 일치하지 않으면 "
                    "실행이 **기본 포트**로 전송됩니다.\n\n"
                    "데드엔드를 방지하려면 기본 포트가 항상 유효한 노드에 연결되도록 하세요."
                )),
                ("사용 팁", (
                    "1. 내장 난이도 분류를 넘어서는 커스텀 분기에 사용하세요.\n"
                    "2. 라우팅 필드는 모든 상태 필드가 가능합니다 (예: `difficulty`, `status`, 커스텀 필드).\n"
                    "3. 열거형 값은 자동으로 문자열로 변환됩니다.\n"
                    "4. 모든 비교는 대소문자를 무시합니다."
                )),
            ],
        ),
    ),
...
ITERATION_GATE_I18N = {
    ...
    "ko": NodeI18n(
        label="반복 게이트",
        description="여러 중지 조건을 확인하는 루프 방지 가드: 반복 횟수, 컨텍스트 예산, 완료 신호, 선택적 커스텀 필드. 한도 초과 시 is_complete=True를 설정합니다.",
        parameters={
            "max_iterations_override": {
                "label": "최대 반복 오버라이드",
                "description": "전역 최대 반복을 오버라이드합니다. 0 = 기본값 사용.",
            },
            "check_iteration": {
                "label": "반복 한도 확인",
                "description": "반복 카운터 대비 확인을 활성화합니다.",
            },
            "check_budget": {
                "label": "컨텍스트 예산 확인",
            ...
            "custom_stop_field": {
                "label": "커스텀 중지 필드",
                "description": "확인할 추가 상태 필드. 참이면 게이트가 중지합니다. 비워두면 비활성화.",
            },
        },
        output_ports={
            "continue": {"label": "계속", "description": "루프 진행 가능"},
            "stop": {"label": "중지", "description": "한도 초과, 루프 종료"},
        },
        groups={"behavior": "동작", "checks": "검사"},
        help=_help(
            "반복 게이트 노드",
            "반복 횟수, 컨텍스트 예산, 완료 신호를 확인하여 무한 루프를 방지하는 안전 게이트입니다.",
            [
                ("개요", (
                    "반복 게이트는 폭주 실행을 방지하는 핵심 **안전 노드**입니다. "
                    "세 가지 조건을 확인합니다:\n\n"
                    "1. **반복 한도** — 루프가 최대 허용 반복을 초과했는가?\n"
                    "2. **컨텍스트 예산** — 컨텍스트 윈도우가 오버플로우 또는 블록 상태인가?\n"
                    "3. **완료 신호** — 이전 노드가 완료를 신호했는가?\n\n"
                    "어떤 조건이든 트리거되면 실행이 **중지** 포트로 라우팅됩니다."
                )),
                ("반복 추적", (
                    "각 루프 반복마다 `iteration` 상태 카운터가 증가합니다. "
                ...
                ("컨텍스트 예산 확인", (
                    ...
                    "Context Guard 노드와 함께 작동합니다."
                )),
                ("사용 팁", (
                    "1. 무한 실행을 방지하려면 모든 루프 시작 부분에 배치하세요.\n"
                    "2. TODO 실행이 반복되는 hard 경로에 필수적입니다.\n"
...
CHECK_PROGRESS_I18N = {
    ...
    "ko": NodeI18n(
        label="진행 확인",
        description="구성 가능한 리스트 필드의 완료 진행을 확인합니다. 항목이 남아있으면 'continue', 모두 처리되면 'complete'로 라우팅합니다. 완료 신호와 오류 플래그를 존중합니다.",
        parameters={
            "list_field": {
                "label": "리스트 상태 필드",
                "description": "진행을 확인할 리스트가 포함된 상태 필드.",
            },
            "index_field": {
                "label": "인덱스 상태 필드",
                "description": "리스트의 현재 인덱스를 추적하는 상태 필드.",
            },
            "completed_status": {
                "label": "완료 상태 값",
                "description": "항목을 완료로 간주하는 상태 값.",
            },
            "failed_status": {
                "label": "실패 상태 값",
                "description": "항목을 실패로 간주하는 상태 값.",
            },
        },
        output_ports={
            "continue": {"label": "계속", "description": "남은 TODO 항목 있음"},
            "complete": {"label": "완료", "description": "모든 TODO 완료"},
        },
        groups={"state_fields": "상태 필드", "behavior": "동작"},
        help=_help(
            "진행 확인 노드",
            "TODO 리스트 완료를 확인하고 continue 또는 complete로 라우팅합니다.",
            [
                ("개요", (
                    "진행 확인 노드는 hard 경로에서 TODO 리스트 완료를 추적하는 "
                    "**조건부 노드**입니다. TODO 리스트 상태를 검사하고 "
                    "실행할 항목이 더 있는지 판단합니다.\n\n"
                    "**라우팅:**\n"
                    "- **계속** — 남은 TODO 항목 있음\n"
                    "- **완료** — 모든 TODO 완료 (또는 완료 신호 수신)"
                )),
                ("진행 추적", (
                    "노드가 읽는 값:\n"
                    "- `current_todo_index` — 다음 TODO 항목\n"
                    "- `todos` — 전체 TODO 리스트\n"
                    "- `completion_signal` — 외부 완료 신호\n\n"
                    "진행 메트릭도 `metadata` 상태 필드에 기록합니다."
                )),
                ("사용 팁", (
                    "1. Execute TODO와 Post Model 노드 뒤에 배치하세요.\n"
                    "2. 'Continue'를 Execute TODO 시퀀스로 다시 연결하세요.\n"
                    "3. 'Complete'를 Final Review 또는 Final Answer에 연결하세요.\n"
                    "4. 매개변수 불필요 — 전적으로 상태에서 작동합니다."
                )),
            ],
        ),
    ),
...
STATE_SETTER_I18N = {
    ...
    "ko": NodeI18n(
        ...
        parameters={
            "state_updates": {
                "label": "상태 업데이트 (JSON)",
                "description": '상태 필드 업데이트 JSON 객체. 예: {"is_complete": true, "review_count": 0}',
            },
        },
        output_ports={"default": {"label": "다음"}},
        groups={"general": "일반"},
        help=_help(
            "상태 설정기 노드",
            "구성된 값으로 상태 필드를 직접 조작합니다.",
            [
                ("개요", (
                    "상태 설정기는 특정 상태 필드를 구성된 값으로 설정하는 유틸리티 노드입니다. "
                    "모델을 사용하지 않으며 순수 상태 조작 노드입니다.\n\n"
                    "상태 초기화, 카운터 리셋, 플래그 설정에 유용합니다."
                )),
                ("상태 업데이트 JSON", (
                    "각 키가 상태 필드 이름이고 각 값이 설정할 값인 JSON 객체를 제공합니다:\n\n"
                    "```json\n{\n  \"is_complete\": true,\n  \"review_count\": 0,\n  \"current_step\": \"reset\"\n}\n```\n\n"
                    "지원 타입: 문자열, 숫자, 불린, null, 배열, 객체."
                )),
                ("사용 팁", (
                    "1. 워크플로우 시작 시 상태를 초기화하는 데 사용하세요.\n"
                    "2. 루프 전에 카운터를 리셋하는 데 사용하세요.\n"
                    "3. `is_complete = true`로 설정하여 워크플로우를 강제 완료할 수 있습니다.\n"
...
MEMORY_INJECT_I18N = {
    ...
    "ko": NodeI18n(
        label="메모리 주입",
        description="LLM 게이트 메모리 주입. LLM이 입력에 장기 메모리 컨텍스트가 필요한지 판단합니다. 활성화되면 벡터 시맨틱 검색과 키워드 검색을 수행하고 memory_refs(메타데이터)와 memory_context(포맷된 텍스트)를 상태에 주입합니다.",
        parameters={
            "max_results": {
                "label": "최대 메모리 결과",
                "description": "검색 유형당 로드할 최대 메모리 청크 수.",
            },
            "search_chars": {
                "label": "검색 입력 길이",
                "description": "메모리 검색에 사용되는 입력 텍스트의 문자 제한.",
            },
            "max_inject_chars": {
                "label": "최대 주입 문자",
                "description": "상태에 주입할 메모리 컨텍스트의 최대 총 문자 수.",
            },
            "enable_llm_gate": {
                "label": "LLM 게이트 활성화",
                "description": "LLM을 사용하여 메모리 검색 필요 여부를 결정합니다. 비활성화 시 비어있지 않은 입력에 항상 메모리를 검색합니다.",
            },
            "enable_vector_search": {
                "label": "벡터 검색 활성화",
                "description": "FAISS 벡터 시맨틱 검색을 활성화합니다. 비활성화 시 키워드 검색만 사용됩니다.",
            },
            "search_field": {
                "label": "검색 소스 필드",
                "description": "메모리 검색 쿼리로 사용되는 값이 있는 상태 필드.",
            },
        },
        output_ports={"default": {"label": "다음"}},
        ...
        help=_help(
            "메모리 주입 노드",
            "LLM 게이트 메모리 주입 — 모델 자체가 장기 메모리 검색 필요 여부를 판단합니다.",
            [
                ("개요", (
                    "메모리 주입 노드는 워크플로우를 세션의 **장기 및 단기 메모리**에 연결합니다. "
                    "하드코딩된 규칙 대신 경량 구조화 출력 호출을 통해 **LLM 자체가** "
                    "입력에 메모리 검색이 필요한지 판단합니다.\n\n"
                    "활성화되면 구조화된 메타데이터(`memory_refs`)와 포맷된 컨텍스트 텍스트"
                    "(`memory_context`)를 상태에 로드합니다. 다운스트림 모델 노드의 프롬프트 템플릿에서 "
                    "`{memory_context}`를 참조하여 자동 주입할 수 있습니다."
                )),
                ("LLM 게이트", (
                    "노드가 LLM에 묻습니다: *\"이 입력에 장기 메모리 컨텍스트가 필요한가?\"*\n\n"
                    "LLM이 고려하는 사항:\n"
                    "- 입력이 과거 작업이나 이전 컨텍스트를 참조하는지\n"
                    "- 작업이 메모리의 도움을 받을 만큼 복잡한지\n"
                    "- 컨텍스트가 필요 없는 간단한 인사/확인인지\n\n"
                    "게이트 실패 시(예: 모델 오류) 안전 기본값으로 메모리를 검색합니다. "
                    "매번 메모리를 검색하려면 게이트를 비활성화하세요."
                )),
                ("메모리 검색 파이프라인", (
                    "LLM 게이트가 검색을 승인하면 다단계 파이프라인이 실행됩니다:\n\n"
                    "1. **세션 요약** — 최신 단기 메모리 상태\n"
                    "2. **MEMORY.md** — 영구 장기 노트\n"
                    "3. **벡터 검색** — FAISS 시맨틱 유사도 검색 (활성화 시)\n"
                    "4. **키워드 검색** — 전통적 키워드 기반 회상\n\n"
                    "결과는 문자 예산(`max_inject_chars`) 내에서 결합됩니다."
                )),
                ("사용 팁", (
                    "1. Start 바로 다음인 워크플로우 맨 앞에 배치하세요.\n"
                    "2. 세션에 메모리 매니저가 구성되어 있어야 합니다.\n"
                    "3. 다운스트림 프롬프트 템플릿에서 `{memory_context}`를 사용하여 검색된 컨텍스트를 주입하세요.\n"
                    "4. 더 풍부한 컨텍스트가 필요한 작업에는 max_results를 늘리세요.\n"
                    "5. 임베딩 모델이 구성되지 않았다면 벡터 검색을 비활성화하세요.\n"
                    "6. 항상 메모리를 검색하려면 LLM 게이트를 비활성화하세요 (디버깅에 유용)."
                )),
            ],
        ),
    ),
...
TRANSCRIPT_RECORD_I18N = {
    ...
    "ko": NodeI18n(
        label="트랜스크립트 기록",
        description="구성 가능한 메시지 역할로 상태 필드의 내용을 단기 메모리 트랜스크립트에 기록합니다. PostModel의 내장 기록이 불충분할 때 명시적 트랜스크립트 제어에 사용합니다.",
        parameters={
            "max_length": {
                "label": "최대 내용 길이",
                "description": "출력에서 기록할 최대 문자 수.",
            },
            "source_field": {
                "label": "소스 상태 필드",
                "description": "트랜스크립트에 기록될 내용이 있는 상태 필드.",
            },
            "role": {
                "label": "메시지 역할",
                "description": "트랜스크립트 항목의 역할 레이블.",
            },
        },
        output_ports={"default": {"label": "다음"}},
        groups={"behavior": "동작", "state_fields": "상태 필드"},
        help=_help(
            ...
            [
                ("개요", (
                    "트랜스크립트 기록 노드는 최신 모델 출력을 세션의 "
                    "단기 메모리에 저장합니다. 이는 향후 턴에서 Memory Inject가 "
                    "사용할 수 있는 대화 기록을 구축합니다.\n\n"
                    "이것은 Post Model이 자동으로 수행하는 트랜스크립트 기록의 "
                    "독립 노드 버전입니다."
                )),
                ("내용 길이", (
                    "**최대 내용 길이** 매개변수는 기록되는 문자 수를 제한합니다.\n\n"
                    "기본값: 5000자. 긴 출력의 경우 처음 N자만 저장됩니다. "
                    "상세하고 중요한 출력을 생성하는 작업에는 이 값을 늘리세요."
                )),
                ("사용 팁", (
                    "1. Post Model 없이 명시적 트랜스크립트 기록이 필요할 때 사용하세요.\n"
                    "2. 모든 모델 노드 뒤에 배치하여 출력을 기록하세요.\n"
                    "3. 세션에 메모리 매니저가 구성되어 있어야 합니다.\n"
                    "4. 메모리 매니저가 없으면 노드가 조용히 아무것도 하지 않습니다."
                )),
            ],
        ),
    ),
...
MEMORY_REFLECT_I18N = {
    "en": NodeI18n(
        ...
        description="Analyzes execution results with the LLM to extract reusable insights and save them as structured memory notes. Place at the end of a workflow to automatically capture learnings.",
        parameters={
            "input_field": {
            ...
            "output_field": {
                "label": "Output State Field",
                "description": "State field containing the execution output to reflect on.",
            },
            "max_insights": {
                "label": "Max Insights",
        ...
        help=_help(
            "Memory Reflect Node",
            "LLM-powered post-execution reflection that extracts reusable knowledge into structured memory notes.",
            [
                ("Overview", (
                    "The Memory Reflect node sits at the end of the workflow and uses the LLM to "
                    "analyze what happened during execution. It extracts reusable insights, decisions, "
                    "and knowledge, then saves them as structured memory notes with tags and categories.\n\n"
                    "This is the **learning** counterpart to Memory Inject (which is the **recall** node)."
                )),
                ("How It Works", (
                    "1. Reads the original input and final output from state\n"
                    "2. Sends both to the LLM with a reflection prompt\n"
                    "3. LLM returns structured JSON with extracted insights\n"
                    "4. Each insight is saved as a memory note with frontmatter\n"
                    "5. Backlinks to related existing notes are created automatically"
                )),
                ("Usage Tips", (
                    "1. Place at the very end of the workflow, just before the End node.\n"
                    "2. Especially useful for the hard path where complex work is done.\n"
                    "3. The memory manager must be configured for the session.\n"
                    "4. Insights are tagged and categorized automatically by the LLM.\n"
                    "5. Adjust max_insights to control how many notes are created per run."
                )),
            ],
        ),
    ),
    "ko": NodeI18n(
        label="Memory Reflect",
        description="LLM을 사용하여 실행 결과를 분석하고 재사용 가능한 인사이트를 구조화된 메모리 노트로 저장합니다. 워크플로우 끝에 배치하여 자동으로 학습 내용을 캡처합니다.",
        parameters={
            "input_field": {
                "label": "입력 상태 필드",
                "description": "원본 사용자 입력이 포함된 상태 필드.",
            },
            "output_field": {
                "label": "출력 상태 필드",
                "description": "반영할 실행 출력이 포함된 상태 필드.",
            },
            "max_insights": {
                "label": "최대 인사이트 수",
                "description": "실행당 추출할 최대 인사이트 수.",
            },
        },
        output_ports={"default": {"label": "다음"}},
        groups={"behavior": "동작", "state_fields": "상태 필드"},
        help=_help(
            "Memory Reflect 노드",
            "LLM 기반 실행 후 반영을 통해 재사용 가능한 지식을 구조화된 메모리 노트로 추출합니다.",
            [
                ("개요", (
                    "Memory Reflect 노드는 워크플로우 끝에 위치하여 LLM을 사용해 실행 중 발생한 "
                    "내용을 분석합니다. 재사용 가능한 인사이트, 결정, 지식을 추출한 후 태그와 카테고리가 "
                    "포함된 구조화된 메모리 노트로 저장합니다.\n\n"
                    "이 노드는 Memory Inject(회상 노드)의 **학습** 대응 노드입니다."
                )),
                ("작동 방식", (
                    "1. 상태에서 원본 입력과 최종 출력을 읽습니다\n"
                    "2. 반영 프롬프트와 함께 LLM에 전송합니다\n"
                    "3. LLM이 추출된 인사이트와 함께 구조화된 JSON을 반환합니다\n"
                    "4. 각 인사이트는 프론트매터가 포함된 메모리 노트로 저장됩니다\n"
                    "5. 관련 기존 노트에 대한 백링크가 자동으로 생성됩니다"
                )),
                ("사용 팁", (
                    "1. End 노드 바로 앞, 워크플로우 맨 끝에 배치하세요.\n"
                    "2. 복잡한 작업이 수행되는 hard 경로에 특히 유용합니다.\n"
                    "3. 세션에 메모리 매니저가 구성되어 있어야 합니다.\n"
                    "4. 인사이트는 LLM에 의해 자동으로 태그 및 분류됩니다.\n"
                    "5. max_insights를 조정하여 실행당 생성되는 노트 수를 제어하세요."
                )),
            ],
        ),
    ),
...
CONTEXT_GUARD_I18N = {
    ...
    "ko": NodeI18n(
        label="컨텍스트 가드",
        description="모델 호출 전 컨텍스트 윈도우 토큰 예산을 확인합니다. 축적된 메시지에서 토큰 사용량을 추정하고 예산 상태를 상태에 기록합니다. 다운스트림 노드가 이를 읽어 여유가 없을 때 프롬프트를 압축합니다.",
        parameters={
            "position_label": {
                "label": "위치 레이블",
                "description": "로깅용 설명 레이블 (예: 'classify', 'execute').",
            },
            "messages_field": {
                "label": "메시지 상태 필드",
                "description": "측정할 메시지 리스트가 포함된 상태 필드.",
            },
        },
        output_ports={"default": {"label": "다음"}},
        ...
        help=_help(
            "컨텍스트 가드 노드",
            "컨텍스트 윈도우 토큰 예산을 모니터링하고 한도에 접근하면 경고하거나 차단합니다.",
            [
                ("개요", (
                    "컨텍스트 가드 노드는 모델의 컨텍스트 윈도우가 얼마나 소비되었는지 "
                    "모니터링하는 **복원력 인프라** 노드입니다.\n\n"
                    "축적된 메시지에서 토큰 사용량을 추정하고 예산 상태를 상태에 기록합니다. "
                    "다운스트림 노드가 이를 읽어 프롬프트를 압축하거나, "
                    "내용을 잘라내거나, 호출을 완전히 건너뛸 수 있습니다."
                )),
                ("예산 상태", (
                    "노드가 상태에 기록하는 `context_budget` 객체:\n\n"
                ...
                ("사용 팁", (
                    "1. 컨텍스트 사용량을 모니터링하려면 모든 모델 노드 앞에 배치하세요.\n"
                    "2. 워크플로우의 다른 지점에 여러 가드를 배치할 수 있습니다.\n"
                    "3. Iteration Gate도 context_budget을 확인합니다 — 함께 작동합니다.\n"
                    "4. 어떤 가드가 트리거되었는지 식별하려면 다른 위치 레이블을 사용하세요."
                )),
            ],
        ),
    ),
...
POST_MODEL_I18N = {
    ...
    "ko": NodeI18n(
        label="모델 후처리",
        description="모든 모델 호출 후에 배치되는 후처리 노드. 수행 내용: (1) 반복 카운터 증가, (2) 선택적 완료 신호 감지, (3) 선택적 트랜스크립트 기록. 필수 복원력 인프라.",
        parameters={
            "detect_completion": {
                "label": "완료 신호 감지",
            ...
            "increment_field": {
                "label": "반복 카운터 필드",
                "description": "반복 카운터로 증가시킬 상태 필드.",
            },
            "source_field": {
                "label": "소스 상태 필드",
                "description": "신호 감지 및 트랜스크립트 기록을 위해 읽을 상태 필드.",
            },
        },
        output_ports={"default": {"label": "다음"}},
        groups={"behavior": "동작", "state_fields": "상태 필드"},
        help=_help(
            ...
            [
                ("개요", (
                    "모델 후처리 노드는 모든 모델 노드 뒤에 배치해야 하는 **복원력 인프라** 노드입니다. "
                    "세 가지를 처리합니다:\n\n"
                    "1. **반복 증가** — 이루어진 모델 호출 수를 추적\n"
                    "2. **완료 신호 감지** — 출력에서 구조화된 신호를 파싱\n"
                    "3. **트랜스크립트 기록** — 출력을 단기 메모리에 저장"
                )),
                ("완료 신호", (
                    "노드가 모델 출력에서 구조화된 완료 신호를 스캔합니다:\n\n"
                ...
                ("트랜스크립트 기록", (
                    "활성화되면 모델 출력을 세션의 단기 메모리 트랜스크립트에 기록합니다. "
                    "최대 5000자까지 저장됩니다.\n\n"
                    "Memory Inject가 사용할 대화 기록을 구축합니다."
                )),
                ("사용 팁", (
                    "1. 모든 모델 노드 뒤에 배치하세요 (LLM Call, Answer, Direct Answer 등).\n"
                    "2. 완전한 복원력을 위해 두 기능 모두 활성화된 상태를 유지하세요.\n"
                    "3. 메모리를 절약하려면 중간 단계에서 트랜스크립트 기록을 비활성화하세요.\n"
                    "4. Iteration Gate가 올바르게 작동하려면 반복 횟수가 중요합니다."
                )),
            ],
        ),
    ),
...
CREATE_TODOS_I18N = {
    ...
    "ko": NodeI18n(
        label="TODO 생성",
        description="LLM을 통해 복잡한 작업을 구조화된 JSON TODO 리스트로 분해합니다. Pydantic 검증된 구조화 출력으로 신뢰성 있는 파싱을 수행하고 TodoItem 형식으로 변환하며 항목 수를 제한하여 폭주 실행을 방지합니다.",
        parameters={
            "prompt_template": {
                "label": "프롬프트 템플릿",
                "description": "TODO 리스트 생성을 위한 프롬프트 템플릿.",
            },
            "max_todos": {
                "label": "최대 TODO 항목",
                "description": "폭주 실행을 방지하기 위한 최대 TODO 항목 수.",
            },
            "output_list_field": {
                "label": "출력 리스트 필드",
                "description": "생성된 리스트를 저장할 상태 필드.",
            },
            "output_index_field": {
                "label": "출력 인덱스 필드",
                "description": "현재 인덱스 상태 필드 (0으로 리셋됨).",
            },
        },
        output_ports={"default": {"label": "다음"}},
        ...
        help=_help(
            "TODO 생성 노드",
            "복잡한 작업을 단계별 실행을 위한 구조화된 TODO 리스트로 분해합니다.",
            [
                ("개요", (
                    "TODO 생성 노드는 **hard 경로**의 진입점을 처리합니다. "
                    "사용자의 복잡한 작업을 모델에 전송하면 모델이 이를 "
                    "구조화된 JSON TODO 리스트로 분해합니다.\n\n"
                    "각 TODO 항목은 ID, 제목, 설명을 가집니다. "
                    "항목들은 이후 Execute TODO 노드에 의해 순차적으로 실행됩니다."
                )),
                ("TODO 형식", (
                    "모델은 Pydantic 스키마로 검증되는 **구조화 JSON 출력**을 생성해야 합니다:\n\n"
                    '```json\n{"todos": [{"id": 1, "title": "1단계", "description": "X 수행"}, '
                    '{"id": 2, "title": "2단계", "description": "Y 수행"}]}\n```\n\n'
                    "노드는 순수 JSON 배열도 처리합니다 (자동으로 스키마에 래핑). "
                    "초기 응답이 검증에 실패하면 자동 수정 재시도가 시도됩니다."
                )),
                ("최대 TODO 항목", (
                    "**최대 TODO 항목** 매개변수는 폭주 실행을 방지하기 위해 리스트를 제한합니다.\n\n"
                    "기본값: 20. 모델이 한도보다 많은 항목을 생성하면 "
                    "처음 N개만 유지됩니다."
                )),
                ("사용 팁", (
                    "1. Classify Difficulty의 'Hard' 포트에서 연결하세요.\n"
                    "2. Iteration Gate → Execute TODO 루프를 뒤에 배치하세요.\n"
                    "3. TODO 리스트는 `todos` 상태 필드에 저장됩니다.\n"
                    "4. 도메인 특화 분해를 위해 프롬프트를 커스터마이즈하세요.\n"
...
EXECUTE_TODO_I18N = {
    ...
    "ko": NodeI18n(
        label="TODO 실행",
        description="계획에서 단일 TODO 항목을 실행합니다. 항목의 컨텍스트와 예산 인식 이전 결과로 프롬프트를 구성합니다. 항목을 완료 또는 실패로 표시하고 인덱스를 진행합니다. CheckProgress와 함께 루프에서 사용하도록 설계되었습니다.",
        parameters={
            "prompt_template": {
                "label": "프롬프트 템플릿",
                "description": "TODO 항목 실행을 위한 프롬프트.",
            },
            "list_field": {
                "label": "리스트 상태 필드",
                "description": "TODO 리스트가 포함된 상태 필드.",
            },
            "index_field": {
                "label": "인덱스 상태 필드",
            ...
            "max_context_chars": {
                "label": "최대 컨텍스트 문자",
                "description": "컨텍스트 윈도우에서 이전 결과당 최대 문자 수.",
            },
            "compact_context_chars": {
                "label": "압축 컨텍스트 문자",
                "description": "컨텍스트 예산이 부족할 때 이전 결과당 최대 문자 수.",
            },
        },
        output_ports={"default": {"label": "다음"}},
        ...
        help=_help(
            "TODO 실행 노드",
            "한 번에 하나의 TODO 항목을 실행하며 이전에 완료된 항목의 결과를 통합합니다.",
            [
                ("개요", (
                    "TODO 실행 노드는 Create TODOs가 생성한 TODO 리스트의 "
                    "개별 항목을 처리합니다. 호출마다 하나의 항목을 실행하고 "
                    "`current_todo_index`를 진행합니다.\n\n"
                    "각 실행은 이전에 완료된 항목의 컨텍스트를 포함하여 "
                    "일관되고 점진적인 작업을 보장합니다."
                )),
                ("실행 컨텍스트", (
                    "프롬프트에 포함되는 내용:\n"
                    ...
                    "- `{previous_results}` — 이미 완료된 항목의 결과\n\n"
                    "컨텍스트 예산이 부족하면 이전 결과가 잘립니다."
                )),
                ("상태 업데이트", (
                    "실행 후:\n"
                    "- TODO 항목의 상태가 `completed`로 설정됩니다 (오류 시 `failed`)\n"
                    "- `current_todo_index`가 증가합니다\n"
                    "- 결과가 TODO 항목의 `result` 필드에 저장됩니다"
                )),
                ("사용 팁", (
                    "1. 루프 안에 배치: Iteration Gate → Context Guard → Execute TODO → Post Model → Check Progress.\n"
                    "2. Check Progress가 Iteration Gate(계속) 또는 Final Review(완료)로 다시 라우팅합니다.\n"
                    "3. 컨텍스트 예산이 부족하면 이전 결과가 자동으로 잘립니다.\n"
                    "4. 실패한 TODO 항목은 기록되지만 다음 항목으로 실행이 계속됩니다."
                )),
            ],
        ),
    ),
...
FINAL_REVIEW_I18N = {
    ...
    "ko": NodeI18n(
        label="최종 리뷰",
        description="Pydantic 검증된 구조화 출력으로 모든 완료된 리스트 항목 결과를 종합 리뷰합니다. 전체 품질 평가, 성과 요약, 이슈 식별, 최종 답변 합성을 위한 실행 가능한 권장사항을 제공합니다.",
        parameters={
            "prompt_template": {
                "label": "프롬프트 템플릿",
                "description": "모든 작업의 최종 리뷰를 위한 프롬프트.",
            },
            "list_field": {
                "label": "리스트 상태 필드",
                "description": "리뷰할 리스트가 포함된 상태 필드.",
            },
            "output_field": {
                "label": "출력 상태 필드",
                "description": "포맷된 리뷰 출력을 저장할 상태 필드.",
            },
            "quality_levels": {
                "label": "품질 등급 (JSON)",
                "description": "구조화 출력의 overall_quality 평가에 허용되는 값.",
            },
            "max_item_chars": {
                "label": "항목당 최대 문자",
                "description": "프롬프트에서 리스트 항목 결과당 최대 문자 수.",
            },
            "compact_item_chars": {
                "label": "항목당 압축 문자",
        ...
        help=_help(
            "최종 리뷰 노드",
            "합성 전 모든 완료된 TODO 항목에 대한 종합적인 Pydantic 검증 리뷰를 수행합니다.",
            [
                ("개요", (
                    "최종 리뷰 노드는 **hard 경로** 완료 시퀀스의 일부입니다. "
                    "**구조화 출력**을 사용하여 모든 TODO 결과를 함께 리뷰합니다 — LLM이 "
                    "`FinalReviewOutput` 스키마에 맞는 JSON 객체를 반환해야 합니다.\n\n"
                    "구조화 출력에는 전체 품질, 완료 요약, "
                    "발견된 이슈, 권장사항이 포함됩니다."
                )),
                ("구조화 출력 형식", (
                    "LLM 응답이 다음 스키마로 검증됩니다:\n"
                    "- `overall_quality` — 구성된 품질 등급 중 하나 "
                    "(기본값: excellent / good / needs_improvement / poor)\n"
                    "- `completed_summary` — 달성된 내용 설명\n"
                    "- `issues_found` — 식별된 특정 이슈 목록 (선택사항)\n"
                    "- `recommendations` — 최종 답변을 위한 제안\n\n"
                    "JSON 파싱 실패 시 수정 프롬프트가 자동 전송됩니다."
                )),
                ("품질 등급", (
                    "`quality_levels` 매개변수는 `overall_quality`에 허용되는 값을 제어합니다. "
                    "LLM 응답이 정확히 일치하지 않으면 가장 가까운 값으로 강제됩니다.\n\n"
                    "다양한 리뷰 기준 커스터마이즈:\n"
                    "- 프로덕션: `[\"pass\", \"fail\"]`\n"
                ...
                ("사용 팁", (
                    "1. Check Progress가 'Complete'로 라우팅한 후에 배치하세요.\n"
                    "2. 합성을 위해 Final Answer를 뒤에 배치하세요.\n"
                    "3. 포맷된 리뷰는 `review_feedback` 상태 필드에 저장됩니다.\n"
                    "4. 품질 메타데이터는 `metadata.review_quality`에 기록됩니다.\n"
                    "5. 이것은 Review 노드와 다릅니다 — 하나의 답변이 아닌 모든 결과를 리뷰합니다."
                )),
            ],
        ),
    ),
...
FINAL_ANSWER_I18N = {
    ...
    "ko": NodeI18n(
        label="최종 답변",
        description="모든 리스트 항목 결과와 리뷰 피드백에서 최종 종합 답변을 합성합니다. 예산 인식 잘림으로 완료된 작업을 결합합니다. 성공 시 워크플로우를 완료로 표시합니다.",
        parameters={
            "prompt_template": {
                "label": "프롬프트 템플릿",
                "description": "최종 답변 합성을 위한 프롬프트.",
            },
            "list_field": {
                "label": "리스트 상태 필드",
                "description": "결과 리스트가 포함된 상태 필드.",
            },
            "feedback_field": {
                "label": "피드백 상태 필드",
                "description": "통합할 리뷰 피드백이 포함된 상태 필드.",
            },
            "output_field": {
                "label": "출력 상태 필드",
        ...
        help=_help(
            ...
            [
                ("개요", (
                    "최종 답변 노드는 hard 경로의 **터미널 노드**입니다. "
                    "모든 TODO 결과와 최종 리뷰 피드백을 가져와 "
                    "단일 종합 응답으로 합성합니다.\n\n"
                    "실행 후 `is_complete`가 True로 설정되어 워크플로우가 종료됩니다."
                )),
                ("합성 내용", (
                    "프롬프트에 포함되는 내용:\n"
                    "- `{input}` — 원본 사용자 요청\n"
                    ...
                    "모델이 이 모든 정보를 일관된 최종 답변으로 결합합니다."
                )),
                ("상태 업데이트", (
                    "실행 후:\n"
                    "- `final_answer` — 합성된 답변\n"
                ...
                ("사용 팁", (
                    "1. Final Review 뒤에 배치하세요.\n"
                    "2. End 노드에 연결하세요 (또는 Post Model → End).\n"
                    "3. Final Review가 실패해도 TODO 결과에서 합성을 시도합니다.\n"
                    "4. 오류 시 원본 TODO 결과를 포함한 대체 답변을 제공합니다."
                )),
            ],
        ),
    ),
...
RELEVANCE_GATE_I18N = {
    ...
    "ko": NodeI18n(
        label="관련성 게이트",
        description="채팅/브로드캐스트 관련성 필터. 경량 LLM 호출로 브로드캐스트 메시지가 에이전트의 역할과 페르소나에 관련 있는지 판단합니다. 관련 없는 메시지는 END로 건너뜁니다.",
        parameters={},
        output_ports={
            "continue": {"label": "계속", "description": "메시지가 관련 있음 — 정상 실행 진행"},
            "skip": {"label": "건너뛰기", "description": "메시지가 관련 없음 — END로 건너뛰기"},
        },
        groups={},
        help=_help(
            "관련성 게이트 노드",
            ...
            [
                ...
                ("라우팅", (
                    ...
                    "- **건너뛰기**: 메시지가 관련 없음 → 빈 출력으로 END 이동"
                )),
                ("사용 팁", (
                    "1. Memory Inject와 Context Guard (Classify) 사이에 배치하세요.\n"
                    "2. 브로드캐스트 메시지에서만 활성화 — 일반 실행에는 투명합니다.\n"
                    "3. 오류 시 정당한 작업을 차단하지 않도록 'relevant'로 기본 설정됩니다."
                )),
            ],
        ),
    ),
...
ADAPTIVE_CLASSIFY_I18N = {
    ...
    "ko": NodeI18n(
        label="적응형 분류",
        description="규칙 기반 빠른 분류 + LLM 폴백. 짧은/사소한 입력은 LLM 호출 없이 즉시 분류되어 8-15초를 절약합니다. 불확실한 입력은 구조화 출력 LLM 분류로 폴백합니다. 인라인 컨텍스트 가드와 모델 후처리를 포함합니다.",
        parameters={
            "prompt_template": {
                "label": "분류 프롬프트",
                "description": "LLM 분류를 위한 프롬프트 (규칙이 불확실할 때만 사용).",
            },
            "categories": {
                "label": "카테고리",
                "description": "쉼표로 구분된 카테고리 이름. 각각이 출력 포트가 됩니다.",
            },
            "default_category": {
                "label": "기본 카테고리",
                "description": "LLM 응답이 어떤 카테고리에도 일치하지 않을 때의 폴백.",
            },
            "output_field": {
                "label": "출력 상태 필드",
                "description": "분류 결과를 저장할 상태 필드.",
            ...
            "enable_rules": {
                ...
                "description": "LLM 전에 규칙 기반 사전 검사를 사용합니다. 비활성화하면 항상 LLM을 사용합니다.",
            },
        },
        output_ports={
            "easy": {"label": "쉬움", "description": "단순하고 직접적인 작업"},
            "tool_direct": {"label": "도구 직접", "description": "직접 도구 실행"},
            "medium": {"label": "보통", "description": "중간 복잡도"},
            "hard": {"label": "어려움", "description": "복잡한 다단계 작업"},
            "extreme": {"label": "매우 어려움", "description": "매우 높은 복잡도"},
            "end": {"label": "종료", "description": "오류 / 조기 종료"},
        },
        groups={"prompt": "프롬프트", "routing": "라우팅", "output": "출력", "behavior": "동작"},
        help=_help(
            "적응형 분류 노드",
            "사소하게 분류 가능한 입력에 대해 LLM 호출을 건너뛰는 규칙 기반 + LLM 하이브리드 분류기입니다.",
            [
                ("개요", (
                    "먼저 빠른 규칙 기반 분류를 시도하는 **조건부 모델 노드**입니다. "
                    "인사, 짧은 사실 질문, 단순 계산은 즉시 'easy'로 분류됩니다. "
                    "규칙으로 카테고리를 결정할 수 없을 때 "
                    "표준 LLM 기반 구조화 출력 분류로 폴백합니다."
                )),
                ("인라인 후크", (
                    "**인라인 컨텍스트 가드** (토큰 추정)와 "
                    "**인라인 모델 후처리** (반복 증가)를 포함하여 "
                    "주변 Guard 및 PostModel 노드의 필요성을 제거합니다."
                )),
                ("사용 팁", (
                    "1. 진입 체인에서 Memory Inject / Relevance Gate 뒤에 배치하세요.\n"
                    "2. 출력 포트를 난이도별 경로에 연결하세요.\n"
                    "3. `enable_rules`를 비활성화하면 모든 입력에 LLM 분류가 강제됩니다.\n"
                    "4. 난이도 외 사용 사례를 위해 카테고리를 커스터마이즈하세요."
                )),
            ],
        ),
    ),
...
DIRECT_TOOL_I18N = {
    ...
    "ko": NodeI18n(
        label="직접 도구",
        description="단발성 도구 실행 노드. 작업의 본질이 도구 운영인 경우 계획 없이 한 번의 LLM 호출로 실행합니다. 실행 후 워크플로우를 완료로 표시합니다.",
        parameters={
            "prompt_template": {
                "label": "도구 실행 프롬프트",
                "description": "LLM에 도구를 직접 실행하도록 지시하는 프롬프트.",
            },
            "output_field": {
                "label": "출력 상태 필드",
...
BATCH_EXECUTE_TODO_I18N = {
    ...
    "ko": NodeI18n(
        label="TODO 일괄 실행",
        description="모든 대기 중인 TODO 항목을 단일 LLM 호출로 실행합니다. N개의 개별 LLM 호출을 1로 줄입니다. 예측 가능하고 독립적인 소수의 TODO 항목이 있는 HARD 경로 작업에 적합합니다.",
        parameters={
            "prompt_template": {
                "label": "일괄 프롬프트",
                "description": "TODO 항목 일괄 실행을 위한 프롬프트. {input}은 목표, {todo_list}는 포맷된 항목에 사용.",
            },
            "list_field": {
                "label": "리스트 상태 필드",
                "description": "TODO 리스트가 포함된 상태 필드.",
...
FINAL_SYNTHESIS_I18N = {
    ...
    "ko": NodeI18n(
        label="최종 합성",
        description="최종 리뷰 + 최종 답변 병합 노드. 모든 완료된 리스트 항목 결과를 리뷰하고 단일 LLM 호출로 완성된 최종 답변으로 합성합니다. 예산 인식 잘림으로 컨텍스트 오버플로를 방지합니다.",
        parameters={
            "prompt_template": {
                "label": "합성 프롬프트",
                "description": "최종 답변 리뷰 및 합성을 위한 프롬프트.",
            },
            "list_field": {
                "label": "리스트 상태 필드",
                "description": "완료된 항목 리스트가 포함된 상태 필드.",
            },
            "output_field": {
                "label": "출력 상태 필드",
                "description": "합성된 답변을 저장할 상태 필드.",
        ...
        help=_help(
            "최종 합성 노드",
            "모든 TODO 결과를 단일 LLM 호출로 하나의 종합 답변으로 리뷰하고 합성합니다.",
            [
                ("개요", (
                    "최종 합성 노드는 **Final Review**와 **Final Answer**의 책임을 "
                    "단일 LLM 호출로 병합합니다. 프롬프트가 모델에 다음을 지시합니다:\n"
                    "1. 완료된 작업의 품질을 리뷰합니다.\n"
                    "2. 모든 결과를 완성된 최종 응답으로 합성합니다.\n\n"
                    "이를 통해 hard 경로에서 LLM 왕복 1회(10-20초)를 절약합니다."
                )),
                ("예산 인식", (
                    "컨텍스트 예산이 부족할 때(block/overflow), 각 항목의 결과가 "
                    "`max_item_chars` 대신 `compact_item_chars`로 잘립니다."
                )),
                ("사용 팁", (
                    "1. hard 경로에서 Check Progress 뒤에 배치하세요.\n"
                    "2. End 노드에 직접 연결하세요 — Post Model 불필요.\n"
                    "3. 6노드 체인을 대체: guard_fr → fin_rev → post_fr → guard_fa → fin_ans → post_fa."
                )),
            ],
        ),
    ),
